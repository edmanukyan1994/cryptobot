...
### `main.py`
Точка входа. Запускает 10 модулей асинхронно:
- collector, ws_monitor, features, forecaster, **forecast_scorer**, trader, market_context, tg_commands, api
...
Дополнительно: `run_forecast_scorer()` заполняет `crypto_forecast_scores` для прогнозов, у которых уже наступил горизонт (1h/4h/24h).
...
Дополнительно: веса из БД нормализуются (отсутствующие факторы добираются из дефолта), порог входа ограничивается диапазоном 20..85. Если вес `ml_signal` = 0, направление ML не блокирует вход.
...
Имеет fallback URL-цепочку (internal + public), retry и поддержку переменных окружения `ML_AGENT_URLS` / `ML_AGENT_URL` для устойчивости.
...
### `sql/analytics_trades.sql`
Готовые запросы к PostgreSQL: KPI по закрытым сделкам, разрезы по `close_reason`, `setup_type`, `trade_type`, `market_mode` из `features_snapshot`, топ символов, а также разрезы по `entry_reason` (причина входа, сохранённая в `features_snapshot`). Запуск: `psql $DATABASE_URL -f sql/analytics_trades.sql`.

### `api.py`
FastAPI эндпоинты для дашборда:
- `/api/status` — баланс, PnL, открытые/закрытые, Fear&Greed, `strategy_mode`, `kill_switch`
- `/api/positions` — открытые позиции (в т.ч. `sl_price`, `setup_type`, `market_mode`, `sr_signal`, `dist_to_sl_pct`)
- `/api/trades` — история (`limit`, опционально `days`), поля `setup_type`, `market_mode`, `sr_signal`, `hold_hours`
- `/api/trades_export.csv` — экспорт CSV (`?days=` опционально)
- `/api/balance_history` — кумулятивный баланс по часам (`?days=` — окно; старт от `initial_balance` аккаунта)
- `/api/daily_pnl` — суммарный PnL по календарным дням (`days`, по умолчанию 90)
- `/api/stats` — win rate, expectancy, profit factor, max drawdown, сырые и **агрегированные** причины выхода (`?days=`)
...
Bybit WebSocket/REST
   ...
   forecaster.py → crypto_forecast_runs (БД)
       ↓
   run_forecast_scorer() → crypto_forecast_scores (БД)
        ↓
   trader.py
   ├── check_entry() — фильтры
   ├── scoring.py — скор
...
**Реализация в `check_entry()`:** жёсткий SR-фильтр (лонг только от `bounce_support` / `breakout_up`, шорт только от `bounce_resistance`) применяется к режимам **`bear_sideways` и `sideways`**. Ранее нейтральный `sideways` в коде не требовал отскока к уровню — это было приведено в соответствие с таблицей (2026-04-20).
...
### Дополнительные фильтры качества входа (`check_entry`)

- Для `setup_type=normal`: требуется `forecast_probability ≥ 55` и `scoring_score ≥ 44`
- Для шортов в `normal/short_trend`: блок при `RSI < 35`, `relative_strength > 1.20`, `forecast_probability < 54`
- Эти пороги выбраны как мягкие (чтобы не «задушить» поток сделок) и пересматриваются по факту статистики
...
Направление задаётся функцией **`detect_direction()`** в `trader.py`: суммируются «голоса» (пробои SR, свечные паттерны, RSI, Фибоначчи, FVG, market structure, импульс 1h, relative strength, forecaster при prob ≥ 60). Минимальный перевес `bull_score - bear_score` ≥ 12; при сильном противоречии свечного скора (`candle_score_long/short < -15`) вход блокируется. SR-отскоки в этом счёте очков не дают (нестабильны между обновлениями), пробои SR по-прежнему сильно влияют. После выбора направления срабатывают фильтры по `market_mode`, SR (см. таблицу выше) и **`scoring.should_enter()`**.
...
| Фактор | Вес | Описание |
|--------|-----|----------|
| `sr_signal` | **0.30** | SR сигнал — главный фактор |
| `candle_confirmation` | **0.25** | Свечное подтверждение |
| `fvg_fibonacci` | **0.15** | FVG + Фибоначчи |
| `rsi` | 0.12 | Перекупленность/перепроданность |
| `relative_strength` | 0.10 | Сила монеты vs BTC |
| `momentum_1h` | 0.05 | Импульс за 1 час |
| `volume` | 0.03 | Объём торгов |
| `ml_signal` | 0.00 | ML (временно отключён) |

**Порог входа:** 45 очков из 100
...
ML сигнал в скоринге временно отключён (`ml_signal` вес = 0.00): это означает, что ML-прогноз считается, но не блокирует вход по направлению. Для оценки качества прогнозов используется `crypto_forecast_scores`.
...
7. **Калибровка `direction_probability` forecaster'а** — требует регулярной проверки по `crypto_forecast_scores`: высокие значения вероятности не всегда соответствуют более высокой фактической точности.
...
### 2026-04-20
- `check_entry()`: для режима **`sideways`** включены те же SR-ограничения, что и для `bear_sideways` (лонг от поддержки/пробоя вверх, шорт только от сопротивления), чтобы поведение совпадало с таблицей стратегии и снизить входы «в шуме».
- Добавлен файл **`sql/analytics_trades.sql`** — воспроизводимая аналитика по `crypto_demo_trades`.
- Обновлены разделы документации: «Направление входа» (актуальная логика `detect_direction`), пояснение по SR для `sideways`.

### 2026-04-20 (дашборд)
- Дашборд: период анализа (сайдбар), бейджи режима стратегии и kill switch, метрики PF / expectancy / max DD, группы причин выхода, график дневного PnL, расширенные таблицы позиций и сделок, экспорт CSV.
- `balance_history` считает кумулятив от **`initial_balance`** из БД; поддержка окна `days`.

### 2026-04-21
- `ml_client.py`: повышена устойчивость ML вызовов — fallback URL (internal + public), retry, env-конфиг `ML_AGENT_URLS` / `ML_AGENT_URL`.
- `scoring.py`: нормализация весов из БД, защита порога входа (20..85), и явная логика: если `ml_signal` выключен (вес=0), направление ML не блокирует вход.
- `trader.py`: причина входа (`entry_reason`) теперь сохраняется в `features_snapshot` сделки для последующей аналитики.
- `sql/analytics_trades.sql`: добавлены разделы анализа по `entry_reason` и сочетанию `setup_type + entry_reason`.
- Добавлен `run_forecast_scorer()` (forecaster): фактическая оценка прогнозов записывается в `crypto_forecast_scores` после наступления горизонта.
- Смягчены дополнительные фильтры входа (`normal`/short quality), чтобы сохранить поток сделок при контроле качества.
