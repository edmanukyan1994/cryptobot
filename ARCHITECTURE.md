...
> Последнее обновление: 2026-04-20 (дашборд)
...
Готовые запросы к PostgreSQL: KPI по закрытым сделкам, разрезы по `close_reason`, `setup_type`, `trade_type`, `market_mode` из `features_snapshot`, топ символов. Запуск: `psql $DATABASE_URL -f sql/analytics_trades.sql`.
